...
# SESSION SECURITY: Ensure session dies when browser closes
app.config['SESSION_PERMANENT'] = False 

# --- OPTIMIZED CURSOR SETTINGS (NO LAG) ---
...
def get_ssid():
    try:
        if is_mac:
            ...
            match = re.search(r' SSID: (.*)', out.decode('utf-8'))
...
# --- NEW: NATIVE MAC POPUP (NO BLACK SCREEN REQUIRED) ---
def show_pin_popup():
    time.sleep(1) # Wait for app to initialize
    ip_addr = get_ip()
    
    if is_mac:
        # AppleScript Native Dialog (এটি কালো স্ক্রিন ছাড়াই কাজ করে)
        msg = f"PIN: {ACCESS_PIN}"
        info = f"Connect: http://{ip_addr}:5000"
        # Force the dialog to appear
        cmd = f'display dialog "{msg}\\n\\n{info}" with title "Magic Tracker by Anupam" buttons {{"START"}} default button "START" with icon note'
        os.system(f"osascript -e '{cmd}'")
    else:
        # Windows Fallback
        try:
            pyautogui.alert(text=f"PIN: {ACCESS_PIN}\nIP: {ip_addr}:5000", title='Magic Tracker', button='OK')
        except: pass
    ...
    def update(self):
        ...
        while self.running:
            try:
                ...
                rel_x = mouse_x - self.monitor["left"]
                rel_y = mouse_y - self.monitor["top"]
...
<body>
    ...
    <div id="settings-panel">
        ...
        <div class="status-grid">
            <div class="status-box full-width"><div><span class="status-label">DEVICE</span><span class="status-val" id="host-val">Loading...</span></div><div><span class="material-icons" style="color:var(--neon-blue)">laptop_mac</span></div></div>
...
def power_action():
    ...
    if is_mac:
        if act == 'shutdown': os.system("osascript -e 'tell app \"System Events\" to shut down'")
        elif act == 'sleep': os.system("osascript -e 'tell app \"System Events\" to sleep'")
    else:
...
def set_volume():
    ...
    if is_mac:
        if action == 'up': os.system("osascript -e 'set volume output volume (output volume of (get volume settings) + 5)'")
        elif action == 'down': os.system("osascript -e 'set volume output volume (output volume of (get volume settings) - 5)'")
    else:
        if action == 'up': pyautogui.press('volumeup') if action == 'up' else pyautogui.press('volumedown')
    return ""
...
@app.route('/zoom')
def zoom_screen():
    d = request.args.get('dir'); key = '+' if d == 'in' else '-'; pyautogui.hotkey(CMD_KEY, key); return ""
@app.route('/type')
...
if __name__ == '__main__':
    # Launch the popup in a separate thread so it doesn't block the server
